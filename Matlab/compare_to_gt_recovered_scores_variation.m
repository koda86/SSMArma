...
for variation_idx = 1:length(variation_levels)
    ...
    % 1) Center each shape, record its centroid‐size, and normalize to unit‐size
    ...
    % 2) Compute the crude mean (unit‐size)
    crudeMean = mean(SyntheticData3D_standardGPA,3);
    
    % 3) Pick the shape closest (in Frobenius distance) to that mean
    ...
    % 4) Initialize meanPoints from that selected unit‐size shape
    meanPoints = SyntheticData3D_standardGPA(:,:,bestIdx);
    
    % 5) Iterative GPA, aligning & re‐scaling each to C_ref every iteration
    iter  = 10;   
    for i = 1:iter
        for f = 1:nShapes
    ...
    %% Constrained Generalized Procrustes Analysis (DK) – revised
    ...
    % 1) Center each shape and record its true centroid‐size,
    ...
    % 2) Compute crude mean (on centered, true‐sized shapes)
    ...
    % 4) Initialize meanPoints from that real‐sized template
    meanPoints = SyntheticData3D_constrainedGPA(:,:,bestIdx);
    
    % 5) Find sole indices once (on centered shapes)
    ...
    % 6) First iteration: zero‐out sole height
    for f = 1:nShapes
    ...
    % 1) Compute the mean‐shape cloud and its centroid
    % meanShapeVec = mean(arrayToRowVector(DataMatrix3D), 1 );  % 1×(3N)
    ...
    % 2) Extract the three PC1 vectors and reshape to N×3
    Dgt  = reshape(ground_truth_pcs(:,1), [], 3);
    Dstd = reshape(coeff_sGPA(:,1),    [], 3);
    Dcon = reshape(coeff_cGPA(:,1),    [], 3);

    % 3) Collapse each field to one 1×3 direction and normalize
    v_gt  = mean(Dgt,  1); v_gt  = v_gt  / norm(v_gt);
    v_std = mean(Dstd, 1); v_std = v_std / norm(v_std);
    v_con = mean(Dcon, 1); v_con = v_con / norm(v_con);

    % 4) Choose arrow length
    arrowLength = 50;    % mm

    % 5) Plot without title or legend
    hFig = figure('Color','w');
    scatter3(meanVerts(:,1), meanVerts(:,2), meanVerts(:,3), 5, 'k', 'filled');
    hold on; axis equal; grid on; view(3); rotate3d on;
